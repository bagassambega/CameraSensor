...
static uint8_t *build_packet(const uint8_t *image, size_t image_size, size_t *out_size)
{
    ...
    size_t total_size = sizeof(timestamp) + sizeof(uint32_t) + image_size;
    ...
    // Copy image size
    uint32_t img_size_32 = (uint32_t)image_size;
    memcpy(packet + sizeof(timestamp), &img_size_32, sizeof(uint32_t));

    // Copy image data
    memcpy(packet + sizeof(timestamp) + sizeof(uint32_t), image, image_size);
...
static void send_image_task(void *pvParameters)
{
    ...
    for (int test = 0; test < num_tests; test++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            uint8_t *packet = build_packet(img.data, img.size, &packet_size);
            ...
            if (packet) {
                ...
                ESP_LOGI(TAG,
                         "[N=%d] Sent [%d/%d] | img_idx=%d | size=%d | msg_id=%d",
                         N,
                         i + 1,
                         N,
                         i % NUM_IMAGES,
